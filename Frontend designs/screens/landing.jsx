// landing.jsx — public landing page (academic project showcase)

const LandingPage = () => (
  <div className="dras-app" style={{display: "flex", flexDirection: "column", overflowY: "auto"}}>
    {/* slim top */}
    <div className="topbar" style={{justifyContent: "space-between"}}>
      <div className="brand">
        <div className="brand-mark">DR</div>
        <span>DRAS</span>
        <span style={{fontSize: 11, color: "var(--text-faint)", marginLeft: 4, fontWeight: 400}}>· Disaster Resource Allocation System</span>
      </div>
      <div style={{display: "flex", gap: 4, alignItems: "center", fontSize: 12, color: "var(--text-muted)"}}>
        <span style={{padding: "4px 10px"}}>Overview</span>
        <span style={{padding: "4px 10px"}}>Architecture</span>
        <span style={{padding: "4px 10px"}}>Schema</span>
        <span style={{padding: "4px 10px"}}>Roles</span>
        <span style={{padding: "4px 10px"}}>Team</span>
        <button className="btn btn-primary btn-sm" style={{marginLeft: 8}}>Sign in</button>
      </div>
    </div>
      ...
      <div style={{display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 64, alignItems: "center"}}>
        <div>
          ...
          <p style={{fontSize: 15, color: "var(--text-muted)", maxWidth: 520, lineHeight: 1.55, margin: 0, marginBottom: 24}}>
            DRAS is a reference database & operations layer that connects control centers, relief camps and warehouses through a single requesting, approval and dispatch pipeline — built on MySQL 8 with full ACID guarantees, role-based access and a stored-procedure-only mutation surface.
          </p>
          <div style={{display: "flex", gap: 8}}>
            <button className="btn btn-primary">Read the implementation plan <Icon name="arrow" size={12}/></button>
            <button className="btn">Open ER diagram</button>
    ...
    <div style={{padding: "48px 64px", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24}}>
      {[
        ...
        {n: "02", t: "Procedure-only mutations", b: "camp_officer and warehouse_mgr cannot directly UPDATE Warehouse_Inventory or Request.Status. Every state change routes through a procedure so triggers, audit and stock alerts always fire."},
    ...
    <div style={{padding: "0 64px 48px"}}>
      ...
      <div style={{display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0, border: "1px solid var(--border)", borderRadius: 6, overflow: "hidden", background: "var(--surface)"}}>
        {[
          {role: "Control Admin", scope: "ALL on reliefchain.*", does: ["Approve / reject requests", "Manage events & camps", "Audit log review"]},
          ...
          <div key={i} style={{padding: 20, borderRight: i < 2 ? "1px solid var(--border)" : "none"}}>
            ...
            {r.does.map((d, j) => (
              <div key={j} style={{display: "flex", gap: 8, alignItems: "center", fontSize: 12, color: "var(--text-muted)", padding: "4px 0"}}>
                <span style={{width: 3, height: 3, borderRadius: 2, background: "var(--accent)"}}/>{d}
              </div>
            ))}
          </div>
        ))}
    ...
    <div style={{padding: "20px 64px", borderTop: "1px solid var(--border)", background: "var(--surface)", display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--text-faint)"}}>
      ...
      <span className="mono">MySQL 8.0.16+ · 13 tables · 4 procedures · 6 views</span>
    </div>
  </div>
);
